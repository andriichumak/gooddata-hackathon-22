...
export const loadDashboard = (dashboardId: string) => async (dispatch: Dispatch) => {
    ...
    const dashboard: DashboardV3EditStackState = {
        ...
        widgets: Object.fromEntries(data.attributes.content.layout.widgets.map((widget: any) => ([widget.uuid, widget]))),
    };
...
export const saveDashboard = () => async (dispatch: Dispatch, getState: () => DashboardV3State) => {
    ...
    if (state.status !== "loaded" || state.saving || state.editStackIndex === state.savedStackIndex)
        return;
...
export const addWidget = (child: DashboardV3WidgetState, parent: DashboardV3WidgetState, index: number, replace = false) => ({
    type: ADD_WIDGET,
    child,
...
export const moveWidget = (child: DashboardV3WidgetState, newParent: DashboardV3WidgetState, newIndex: number, replace = false) => (dispatch: Dispatch) => {
